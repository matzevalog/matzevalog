---
title: "QBA1075"
sidebar: tomb-list
sex: мужской
place: |
    Куба (центральное)

    Азербайджан
name-ru: |
    Элияху, сын Элазара
    
    אליהו בן אלעזר
name-he: |
    אליהו בן אלעזר
    
    Элияху, сын Элазара
year-gr: 1874
year-he: 5634
categories:
  - c:QBA
  - g:мужской
  - tt:стела

  - l:иврит
  
  - m:известняк
  
lightbox: true
format:
  html:
    df-print: kable
include-in-header:
  - text: |
      <style>
         .quarto-title { display: none; }
         .tab-content {
            padding-top: 0.2em;
            padding-bottom: 0.2em;
         }
         .panel-tabset { margin-top: 1em; }
         table tbody tr:nth-of-type(odd) {
            background-color: rgba(0, 0, 0, 0.04) !important;
         }
         table, table tr, table th, table td {
            border: none !important;
            border-bottom: none !important;
            border-top: none !important;
         }
          .name-style p { font-weight: bold; margin-bottom: 0.1em; }
          .sidebar.sidebar-navigation>* { padding-top: 1.1em; }
          #title-block-header { margin-block-end: 0em; }
      </style>
---

::: {style="font-size: 1em; margin-bottom: 1.2em"}
[Куба (центральное)](../cemeteries/QBA.html) > QBA1075
:::

```{r}
suppressPackageStartupMessages(library(tidyverse))
```


:::: {.columns}

::: {.column width="20%"}

```{r}
#| lightbox:
#|   group: photos

knitr::include_graphics("../images/QBA1075_1.JPG")
knitr::include_graphics("../images/QBA1075_2.JPG")

```
:::

::: {.column width="5%"}
:::

::: {.column width="74%"}

::: {.name-style}
Элияху, сын Элазара
:::

::: {style="font-size: 1.2em;"}
אליהו בן אלעזר
:::

:::: {.columns}
::: {.column width="5%"}
![](../additional_images/Cradle.svg){width=1.3em}
:::

::: {.column style="width: 40%; padding-left: 0.2em;"}
  /  
:::

::: {.column width="5%"}
![](../additional_images/Tombstone.svg){width=1.3em}
:::

::: {.column style="width: 50%; padding-left: 0.2em;"}
03.09.1874* / 21.El.5634
:::

::::


::: {.panel-tabset}

#### Эпитафия

```{r}
tibble(`Оригинал` = "פה נטמן
איש נבון ומש
רודף צדקה וחסד
המנוח אליהו בר
אלעזר זל ה״ה יום
חמישי בשבת
שהוא אחד ועשרי׳
לחודש אלול
שנת ה׳ת״רלד
לפ״ג ליצירה",
       `Перевод` = "Здесь покоится
человек мудрый и образованный,
стремящийся к правде и милости,
покойный Элияху, сын р.
Элиэзера, благословенной памяти.
Четверг, 
который 21
числа месяца элул,
года 5634
по большому летоисчислению от сотворения мира.") |>
  mutate(`Оригинал` = str_split(`Оригинал`, "
"),
         `Перевод` = str_split(`Перевод`, "
")) |>
  unnest_longer(c(`Оригинал`, `Перевод`)) |> 
  mutate(id = 1:n()) |> 
  relocate(id, .before = Перевод) |> 
  rename(` ` = id) |> 
  knitr::kable(align = c("r", "c", "l"))
```

|||
|-|---|
|**Примечания**| |
|**Язык**      |иврит    |
|**Метки**     |     |
: {tbl-colwidths="[25,75]"}

#### Памятник

|||
|-|---|
|**Тип памятника**   |стела                                                                         |
|**Материал**        |известняк (сколы, трещины, следы эрозии)                                                     |
|**Размеры**         |129×46×23                                                                        |
|**Тип декора**      |                                                                         |
|**Элементы декора** |фигурная ниша для текста                                                                          |
|**Координаты**      |[41.37044417, 48.50838198](https://www.google.com/maps/place/41.37044417, 48.50838198)                    |
: {tbl-colwidths="[25,75]"}

#### Источник

|||
|-|---|
|**Место хранения**        |Архив полевых исследований Центра «Сэфер»     |
|**Контакты**              |students@sefer.ru    |
|**Ссылка**                |https://sfira.org        |
|**Исходный ID**           |  |
|**Условия использования** |[CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/deed.ru)     |
: {tbl-colwidths="[25,75]"}

:::

:::

::::

